# -*- coding: utf-8 -*-
"""
Created on Fri Jul  3 10:11:49 2020
...
cols=[2,6,7,19,21]#using only columns of name, chain_name
dataset=pd.read_excel('Book3.xlsx',index_col=None,na_values=['NA'],usecols=cols)
dataset=dataset.fillna('')



#Making list1, list2, list3, list4 and list5 with the dataset
list1=dataset['name'].values
list2=dataset['chain_name'].values
list3=dataset['locality'].values
list4=dataset['region'].values
list5=dataset['category_ids'].values
print(list2[0])
...
#filtering out top 30 chain_names based on frequency to solve the memory issue faced on my local machine
chain_names_frequency=dict()
t=0
for item in list1:
    if list2[t] not in chain_names_frequency:
        chain_names_frequency[list2[t]]=1
    else:
        chain_names_frequency[list2[t]]+=1
    t+=1    
#sorting the dictionary in descending order
chain_names_frequency_sorted = dict(sorted(chain_names_frequency.items(), key=operator.itemgetter(1),reverse=True))

top_30_chain_names=dict()
t=0
for item in chain_names_frequency_sorted:
    if t==30:
        break
    top_30_chain_names[item]=chain_names_frequency_sorted[item]
    t+=1
...
#concataneting the merchant name with corresponding locality, region, category_id values and appending them to list called result  
t=0
for item in list1:
    if list2[t] in top_50_chain_names:
        if item+' '+list3[t]+' '+list4[t]+' '+list5[t] not in seen:
            seen.add(item+' '+list3[t]+' '+list4[t]+' '+list5[t])
            result.append(item+' '+list3[t]+' '+list4[t]+' '+list5[t])
    t+=1
...
saver.save(sess,'my_model4')#saving the learned weights
...
for i in vocab:
    embeddings[i]=vectors[char_to_int[i]]
        ...
        #print('running')
    #Z+=embeddings[i]
    #print(embeddings[i]+Z)
#print(Z)

import math
j=len(sentences)#length of list named sentences
words_frequency_global={}#getting words frequency across all the documents(merchant names concatanated string)
...
for sentence in sentences:
    ...
    #document_vector=[0,0]
        words_frequency={}#word frequncy within a merchant name concatanated string
        TF_IDF={}
        n=len(sentence)
        for idx, word in enumerate(sentence):
...
#mapping each of the unique merchant name concatanated string to its corresponding chain_name entry   
merchant_chain_name=dict()
t=0

for item in list1:
    if item+' '+list3[t]+' '+list4[t]+' '+list5[t] in result:
        merchant_chain_name[item+' '+list3[t]+' '+list4[t]+' '+list5[t]]=list2[t]
